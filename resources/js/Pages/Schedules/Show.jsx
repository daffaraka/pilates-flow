...
export default function Show({ schedule, bookedCount, isFull }) {
    ...
    return (
        <AppLayout title="Detail Kelas">
            ...
            <div>
                <Link href={route('schedules.index')} className="text-sm text-text-secondary hover:text-primary mb-6 inline-flex items-center gap-2">
                ...
                <Card>
                    <Card.Header className="bg-primary-dark text-surface">
                        <div className="flex justify-between items-start">
                            <div>
                                <h1 className="text-3xl font-serif mb-2">{schedule.title}</h1>
                                <p className="text-surface/80">{dateStr} • {timeStr}</p>
                            </div>
                    ...
                    <Card.Body className="space-y-6">
                        <div>
                            <h4 className="text-sm text-text-secondary uppercase tracking-wider font-bold mb-2">Instruktur</h4>
                            <p className="text-lg">{schedule.instructor?.user?.name || '-'}</p>
                            <p className="text-text-secondary text-sm">{schedule.instructor?.bio || ''}</p>
                        </div>

                        <div>
                            <h4 className="text-sm text-text-secondary uppercase tracking-wider font-bold mb-2">Lokasi</h4>
                            <p className="text-lg">{schedule.location || 'Studio Utama'}</p>
                        </div>
                    </Card.Body>
                    
                    <Card.Footer className="flex justify-between items-center">
                        <span className="text-sm text-text-secondary">
                            Pastikan Anda memiliki membership aktif untuk memesan.
                        </span>
                        
                        {auth.user ? (
                            <form onSubmit={handleBooking}>
                                <Button disabled={isFull || schedule.status === 'cancelled' || processing}>
